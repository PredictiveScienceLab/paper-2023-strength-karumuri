...
resultdir = os.path.join(os.getcwd(), 'ModelB-Htest_Available_results' ,str(seed)+'-seed') 
...
def posterior_samples_f_test(X, Y, size=100): # gives posterior samples of all test points
    samples = []
    samples_0, samples_1 = np.ndarray((X.shape[0], 1, size)), np.ndarray((X.shape[0], 1, size))
    for i in range(X.shape[0]):
        X_new = X[i,:][None,:] 
        Y_new = Y[i,:][None,:] 
        Ys = posterior_samples_f_test_single(X_new, Y_new, size=size)
        samples_0[i,:,:], samples_1[i,:,:] = Ys[0][0], Ys[1][0]
    samples.append(samples_0)
    samples.append(samples_1)
    return samples

import copy
def posterior_samples_f_test_single(X_new, Y_new, size=100): # gives posterior samples of one test point
    # To store the samples
    Ys = []
    
    m1_copy = copy.deepcopy(m1)
    X_up = np.vstack([m1_copy.X, X_new[:,:m1_copy.X.shape[1]]])
    Y_up = np.vstack([m1_copy.Y, Y_new])
    m1_copy.set_XY(X_up, Y_up)
    
    F1 = m1_copy.posterior_samples_f(X_new[:,:m1_copy.X.shape[1]], size=size) # The _f is required
    Ys.append(F1)
    # To store samples from this fidelity
    Ysi = np.ndarray((X_new.shape[0], 1, size))
    for s in range(size):
        Xsi =  np.hstack(( X_new, Ys[0][:, :, s] ))
        # This is the computationally expensive step:
        Ysi[:, :, s] = m2.posterior_samples_f(Xsi, size=1)[:, :, 0]
    Ys.append(Ysi)
    
    m1_copy.set_XY(m1.X, m1.Y) # clean up
    return Ys 
...
postsam1_train = posterior_samples_f_train(X1_train_s, size=1000)[0] # posterior_samples_f gives epistemic uncertainity only
postsam1_train_un = scaler_y1.inverse_transform(postsam1_train) # un-normalized values 
print (postsam1_train_un.shape)

postsam1_test = posterior_samples_f_test(X_test_s, y1_test_s, size=1000)[0] # posterior_samples_f gives epistemic uncertainity only
...
postsam2_train = posterior_samples_f_train(X2_train_s, size=1000)[1] # posterior_samples_f gives epistemic uncertainity only
postsam2_train_un = scaler_y2.inverse_transform(postsam2_train) # un-normalized values 
print (postsam2_train_un.shape)

postsam2_test = posterior_samples_f_test(X_test_s, y1_test_s, size=1000)[1] # posterior_samples_f gives epistemic uncertainity only
...
plt.plot(y1_test, mupred1_test , 'o', color='blue', markersize=4)
...
plt.errorbar(y1_test, mupred1_test, yerr=errbarpred1_test.flatten(), fmt='o', color='blue',markersize=4)
plt.plot(fit, fit, 'x', label='', color='black', markersize=2)
plt.xlabel('Experimental hardness', fontsize=12)
...
# In[ ]:
# to run 
# python ModelB-Htest_Available.py -seed=23
